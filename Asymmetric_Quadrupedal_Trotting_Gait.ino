 ...
 *   - theta1: hip angle from vertical (+y axis), measured via atan2(x,y)
 *            theta1 = 0° when femur points straight down
 *            theta1 > 0° when femur rotates forward
 *   - theta2: knee angle measured as deflection from fully extended
 *            theta2 = 0° when leg is FULLY EXTENDED (straight)
 *            theta2 = 180° when leg is FULLY FOLDED
 * 
 * Forward Kinematics:
...
// Gait Params
float stride_length = 10.0; // Total horizontal travel per stride (cm)
float stance_depth = 13.0; // Foot depth below hip during stance (cm)
float swing_height = 8.0; // Max lift during swing (cm)
float swing_peak_position = 0.3; // Normalized position of max height in swing (0-1)

float gait_period = 10.0;
...
float getLegPhaseOffset(int leg_index) {
  ...
  switch(leg_index) {
    case 0: return 0.0;
    case 1: return second_pair_offset;
    case 2: return second_pair_offset;
    case 3: return 0.0;
    default: return 0.0;
  }
}
...
void setup() {
  ...
  DEBUG_SERIAL.println("Leg params");
